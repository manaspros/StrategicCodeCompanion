...
export class CLIAnalysisEngine {
    ...
    async analyzeCodebase(codePath: string, useEnhancedFeatures: boolean = true): Promise<AgentResults> {
        const steps = [
            'Scanning codebase',
        ...
        try {
            ...
            const ingestion = new CodeIngestion(codePath);
            ...
            const orchestrator = new MultiAgentOrchestrator(
                llmProvider, 
                useEnhancedFeatures ? this.config.getComposioKey() : undefined
            );
    ...
    private filterAndPrepareChunks(chunks: CodeChunk[]): CodeChunk[] {
