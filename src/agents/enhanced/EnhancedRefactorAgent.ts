...
import { SemanticCodeAnalyzer, SemanticContext } from './SemanticCodeAnalyzer';
import { RefactoringSuggestion, RefactoringResults } from '../RefactorAgent';

export class EnhancedRefactorAgent {
    ...
    private async generateContextAwareSuggestions(
        ...
        const generatedSuggestions = await Promise.all(
            suggestionGenerators.map(generator => generator())
        );
    ...
    private async generateQualityHotspotSuggestions(
        semanticContext: SemanticContext,
        codeChunks: CodeChunk[]
    ): Promise<RefactoringSuggestion[]> {
        const suggestions: RefactoringSuggestion[] = [];
...
Return a specific refactoring suggestion as JSON:
{
  "suggestion": {
    ...
    private async generateArchitecturalPatternSuggestions(
        semanticContext: SemanticContext,
        codeChunks: CodeChunk[]
    ): Promise<RefactoringSuggestion[]> {
        const suggestions: RefactoringSuggestion[] = [];
        ...
        const violatedPatterns = semanticContext.architecturalPatterns.filter(
            pattern => pattern.violations.length > 0
        );
...
Based on the pattern violations, create 1-2 specific refactoring suggestions as JSON:
{
  "suggestions": [
    ...
    private async generateCrossCuttingConcernSuggestions(
        semanticContext: SemanticContext,
        codeChunks: CodeChunk[]
    ): Promise<RefactoringSuggestion[]> {
        const suggestions: RefactoringSuggestion[] = [];
        ...
        const inconsistentConcerns = semanticContext.crossCuttingConcerns.filter(
            concern => concern.implementation === 'inconsistent' || concern.implementation === 'missing'
        );
...
Create 1-2 specific refactoring suggestions to standardize this concern as JSON:
{
  "suggestions": [
    ...
    private async generateBusinessLogicSuggestions(
        semanticContext: SemanticContext,
        codeChunks: CodeChunk[],
        analysis: CodebaseAnalysis
    ): Promise<RefactoringSuggestion[]> {
        ...
        const criticalAreas = semanticContext.businessLogicAreas.filter(
            area => area.businessCriticality === 'critical' && area.complexity > 7
        );
  ...
  "suggestion": {
    ...
    private prioritizeBySemantic(
        ...
        const prioritized = suggestions.map(suggestion => {
            ...
            const affectedBusinessArea = semanticContext.businessLogicAreas.find(area =>
                area.files.some(file => suggestion.filePath?.includes(file))
            );
            ...
            const affectedHotspot = semanticContext.qualityHotspots.find(hotspot =>
                hotspot.filePath === suggestion.filePath
            );
        ...
        return prioritized.sort((a, b) => (b as any).priorityScore - (a as any).priorityScore);
    }
    ...
    private getUpdatedPriority(score: number): 'high' | 'medium' | 'low' {
