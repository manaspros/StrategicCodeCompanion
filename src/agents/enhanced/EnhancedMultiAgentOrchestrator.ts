...
import { CodeChunk, CodebaseAnalysis, AgentResults } from '../main';
import { SemanticCodeAnalyzer } from './SemanticCodeAnalyzer';
import { IntelligentChunkSelector, ChunkSelectionStrategy } from './IntelligentChunkSelector';
import { EnhancedRefactorAgent } from './EnhancedRefactorAgent';

// Import your existing agents
import { ArchitectAgent } from '../ArchitectAgent';
import { LibrarianAgent } from '../LibrarianAgent';
import { TutorAgent } from '../TutorAgent';
import { OpenAIAgentOrchestrator, EnhancedAgentResults } from '../../services/OpenAIAgentOrchestrator';

export interface EnhancedAnalysisResults extends AgentResults {
    semanticContext?: any; // Semantic analysis results
    chunkPriorities?: any[]; // Intelligent chunk selection info
    analysisStrategy?: string; // Strategy used for analysis
    enhancedInsights?: {
        codeFlows: any[];
        qualityHotspots: any[];
        architecturalPatterns: any[];
        businessLogicAreas: any[];
    };
}
...
export class EnhancedMultiAgentOrchestrator {
    ...
    private chunkSelector: IntelligentChunkSelector;
    private enhancedRefactorAgent: EnhancedRefactorAgent;
    
    // Keep existing agents for compatibility
    ...
    constructor(llmProvider: LLMProvider, composioApiKey?: string) {
        ...
        this.semanticAnalyzer = new SemanticCodeAnalyzer(llmProvider);
        this.chunkSelector = new IntelligentChunkSelector(llmProvider);
        this.enhancedRefactorAgent = new EnhancedRefactorAgent(llmProvider);
    ...
    async analyzeCodebase(chunks: CodeChunk[], useEnhanced: boolean = true): Promise<EnhancedAnalysisResults> {
        try {
            ...
            if (useEnhanced) {
                ...
                const results = await this.runEnhancedAnalysis(
                    analysis, 
                    chunkPriorities, 
                    semanticContext
                );
    ...
    private async performIntelligentChunkSelection(chunks: CodeChunk[]) {
        const strategies = {
            refactoring: {
                maxChunks: 30,
                focusArea: 'quality' as const,
                includeContext: true,
                diversityWeight: 0.3
            },
            architecture: {
                maxChunks: 25,
                focusArea: 'architecture' as const,
                includeContext: true,
                diversityWeight: 0.4
            },
            libraries: {
                maxChunks: 20,
                focusArea: 'business-logic' as const,
    ...
    private async runEnhancedAnalysis(
        ...
        const refactoringChunks = chunkPriorities.refactoring.map((p: any) => p.chunk);
        const architectureChunks = chunkPriorities.architecture.map((p: any) => p.chunk);
        const librariesChunks = chunkPriorities.libraries.map((p: any) => p.chunk);
        const tutorialsChunks = chunkPriorities.tutorials.map((p: any) => p.chunk);
    ...
    private async runEnhancedArchitectureAnalysis(
        analysis: CodebaseAnalysis,
        chunks: CodeChunk[],
        semanticContext: any
    ) {
        ...
        if (semanticContext.architecturalPatterns) {
            const enhancedFeatures = baseResults.features?.map((feature: any) => ({
                ...feature,
                semanticInsights: {
                    relatedPatterns: semanticContext.architecturalPatterns
                        .filter((p: any) => p.adherence < 0.7)
                        .map((p: any) => p.pattern),
                    qualityHotspots: semanticContext.qualityHotspots
                        .filter((h: any) => h.riskScore > 7)
                        .length
                }
            }));
    ...
    private async runEnhancedLibraryAnalysis(
        analysis: CodebaseAnalysis,
        chunks: CodeChunk[],
        semanticContext: any
    ) {
        ...
        if (semanticContext.businessLogicAreas) {
            const enhancedRecommendations = baseResults.recommendations?.map((rec: any) => ({
                ...rec,
                businessContext: {
                    relevantDomains: semanticContext.businessLogicAreas
                        .filter((area: any) => area.complexity > 7)
                        .map((area: any) => area.domain),
                    criticalAreas: semanticContext.businessLogicAreas
                        .filter((area: any) => area.businessCriticality === 'critical')
                        .length
                }
            }));
    ...
    private async runEnhancedTutorialAnalysis(
        analysis: CodebaseAnalysis,
        chunks: CodeChunk[],
        semanticContext: any
    ) {
        ...
        if (semanticContext.qualityHotspots) {
            const learningAreas = semanticContext.qualityHotspots
                .filter((h: any) => h.riskScore > 6)
                .map((h: any) => h.issues.map((i: any) => i.type))
                .flat();
            
            const enhancedTutorials = baseResults.tutorials?.map((tutorial: any) => ({
                ...tutorial,
                relevanceContext: {
                    addressesQualityIssues: learningAreas.some((area: string) => 
                        tutorial.topics?.some((topic: string) => 
