...
export interface SemanticContext {
    ...
    businessLogicAreas: BusinessLogicArea[];
...
export interface BusinessLogicArea {
    ...
    businessCriticality: 'critical' | 'important' | 'supporting';
}
...
export interface CrossCuttingConcern {
    concern: 'logging' | 'security' | 'error-handling' | 'validation' | 'caching' | 'monitoring';
    implementation: 'consistent' | 'inconsistent' | 'missing';
    locations: string[];
    recommendations: string[];
}
...
export class SemanticCodeAnalyzer {
    ...
    async analyzeSemanticContext(codeChunks: CodeChunk[]): Promise<SemanticContext> {
    ...
    private async analyzeCodeFlows(codeChunks: CodeChunk[]): Promise<CodeFlow[]> {
...
Return a JSON array of flows:
{
  "flows": [
    ...
    private async analyzeDependencies(codeChunks: CodeChunk[]): Promise<DependencyMap> {
...
Return detailed dependency analysis as JSON:
{
  ...
  "couplingMetrics": [
    ...
    private async analyzeArchitecturalPatterns(codeChunks: CodeChunk[]): Promise<ArchitecturalPattern[]> {
...
Return pattern analysis as JSON:
{
  "patterns": [
    ...
    private async analyzeBusinessLogicAreas(codeChunks: CodeChunk[]): Promise<BusinessLogicArea[]> {
...
Return business area analysis as JSON:
{
  "areas": [
    ...
    private async analyzeQualityHotspots(codeChunks: CodeChunk[]): Promise<QualityHotspot[]> {
...
Return quality analysis as JSON:
{
  "hotspots": [
    ...
    private async analyzeCrossCuttingConcerns(codeChunks: CodeChunk[]): Promise<CrossCuttingConcern[]> {
...
Return concern analysis as JSON:
{
  "concerns": [
    ...
    private buildCodeContext(codeChunks: CodeChunk[], analysisType: string): string {
    ...
    private selectRelevantChunks(codeChunks: CodeChunk[], analysisType: string): CodeChunk[] {
        ...
        switch (analysisType) {
            case 'flows':
                ...
                chunks = chunks.filter(c => 
                    c.filePath.includes('controller') ||
