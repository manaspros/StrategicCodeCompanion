...
import { RateLimitManager, RateLimitConfig, FallbackStrategy } from './enhanced/RateLimitManager';
...
export class LibrarianAgent {
    ...
    constructor(llmProvider?: LLMProvider) {
        ...
        const rateLimitConfig: RateLimitConfig = {
            maxRequestsPerHour: 5000, // GitHub authenticated limit
    ...
    private async searchGitHubReposWithStrategy(searchTerms: string[], query: any): Promise<any[]> {
    ...
    private async processRepositoriesWithStrategy(
        repos: any[], 
    ...
    private async createIntelligentRecommendation(
        repo: any, 
        analysis: CodebaseAnalysis, 
        query: any, 
        dynamicAnalysis: any
    ...
    private calculateIntelligentRelevanceScore(repo: any, analysis: CodebaseAnalysis, query: any, dynamicAnalysis: any): number {
        let score = 0;
        ...
        if (dynamicAnalysis.functionalityGaps) {
            dynamicAnalysis.functionalityGaps.forEach((gap: any) => {
    ...
    private generateIntelligentBenefits(repo: any, category: LibraryRecommendation['category'], query: any, dynamicAnalysis: any): string[] {
        const benefits = [
        ...
        if (dynamicAnalysis.performanceInsights) {
            const relevantInsight = dynamicAnalysis.performanceInsights.find((insight: any) =>
                repo.name.toLowerCase().includes(insight.area.toLowerCase()) ||
    ...
    private generateIntelligentUseCases(repo: any, category: LibraryRecommendation['category'], query: any, dynamicAnalysis: any): string[] {
        ...
        if (dynamicAnalysis.architecturalOpportunities) {
            const relevantOpportunity = dynamicAnalysis.architecturalOpportunities.find((opp: any) =>
                repo.name.toLowerCase().includes(opp.opportunity.toLowerCase().split(' ')[0])
            );
    ...
    private assessIntegrationEffort(repo: any, analysis: CodebaseAnalysis): 'low' | 'medium' | 'high' {
        if (analysis.key_technologies.some(tech => 
    ...
    private async getCachedRecommendations(): Promise<LibraryResults> {
    ...
    private async getStaticRecommendations(): Promise<LibraryResults> {
        console.log('LibrarianAgent: Using static recommendations fallback');
        
        const staticRecommendations: LibraryRecommendation[] = [
            {
                id: 'lodash',
                name: 'lodash',
    ...
    private async getFallbackRecommendationsForQuery(searchTerms: string[], query: any): Promise<any[]> {
