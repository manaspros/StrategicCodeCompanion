...
<body>
    ...
    private async generateBody(results: AgentResults, options: ReportOptions): Promise<string> {
    ...
    private generateExecutiveSummary(results: AgentResults): string {
    ...
    private generateRefactoringSection(results: AgentResults): string {
        ...
        const suggestions = results.refactoring.suggestions.map((suggestion: any) => `
            <div class="recommendation-card">
                ...
                <div style="margin-top: 15px;">
                    ...
                    <ul>
                        ${suggestion.benefits.map((benefit: string) => `<li>${benefit}</li>`).join('')}
                    </ul>
                </div>
            </div>
        `).join('');
    ...
    private generateArchitectureSection(results: AgentResults): string {
        ...
        const features = results.architecture.features.map((feature: any) => `
            <div class="recommendation-card">
                <div class="recommendation-header">
                ...
                <div>
                    ...
                    <div class="tech-tags">
                        ${feature.implementationOverview.technologies.map((tech: string) => `<span class="tech-tag">${tech}</span>`).join('')}
                    </div>
                </div>
                ...
                <div>
                    <h4>Benefits:</h4>
                    <ul>
                        ${feature.benefits.map((benefit: string) => `<li>${benefit}</li>`).join('')}
                    </ul>
                </div>
            </div>
        `).join('');
            ...
            <div class="library-card">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0;">
                    <div>
                        <h4>💡 Key Benefits:</h4>
                        <ul>
                            ${lib.benefits.slice(0, 3).map((benefit: string) => `<li>${benefit}</li>`).join('')}
                        </ul>
                    </div>
                    <div>
                        <h4>🎯 Use Cases:</h4>
                        <ul>
                            ${lib.useCases.slice(0, 3).map((useCase: string) => `<li>${useCase}</li>`).join('')}
                        </ul>
                    </div>
                </div>
        ...
        const tutorials = results.tutorials.tutorials.map((tutorial: any) => {
            ...
            <div class="tutorial-card">
                ...
                <div class="tech-tags" style="margin: 10px 0;">
                    ${tutorial.topics.map((topic: string) => `<span class="tech-tag">${topic}</span>`).join('')}
                </div>
    ...
    private generateUniqueRecommendations(recommendations: any[]): string {
    ...
    private generateCompetitiveAnalysis(analysis: any): string {
        const opportunities = analysis.innovationOpportunities.map((opp: any) => `
            <div class="impact-card">
    ...
    private generateBusinessStrategy(strategy: any): string {
        ...
        <div>
            ...
            <ul>
                ${strategy.marketPositioning.differentiators.map((diff: string) => `<li>${diff}</li>`).join('')}
            </ul>
    ...
    private getComplexityText(complexity: number): string {
        if (complexity <= 3) return '🟢 Low Complexity';
