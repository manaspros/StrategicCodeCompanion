...
class YouTubeSearchService {
     ...
     */
    static searchTutorials(topics, difficulty, limit = 5) {
        ...
        const recommendations = scoredVideos
            ...
            difficulty: this.mapDifficultyFromContent(item.video.title, item.video.description) || difficulty || 'intermediate',
            topics: normalizedTopics,
            relevanceScore: Math.min(0.95, 0.7 + (item.score / 50)),
     ...
     */
    static getTutorialByTopic(topic, difficulty) {
     ...
     */
    static getHighQualityRecommendations(limit = 5) {
        ...
        return highQualityVideos.map(video => ({
            ...
            author: video.author,
            difficulty: this.mapDifficultyFromContent(video.title, video.description) || 'intermediate',
            topics: video.tags,
            relevanceScore: 0.85,
            relatedTo: 'general'
        }));
    }
    static mapDifficultyFromContent(title, description) {
    ...
    static determineRelation(topics) {
...
YouTubeSearchService.curatedVideos = [
