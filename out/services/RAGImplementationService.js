...
exports.RAGImplementationService = void 0;
const RAGContextBuilder_1 = require("../utils/RAGContextBuilder");
class RAGImplementationService {
    constructor(llmProvider) {
        this.llmProvider = llmProvider;
    }
    setCodebaseContext(analysis, codeChunks) {
     ...
     */
    async generateFeatureImplementation(feature) {
        ...
        const ragContext = RAGContextBuilder_1.RAGContextBuilder.buildComprehensiveContext(this.currentAnalysis, this.currentCodeChunks);
        ...
        const implementationPrompt = this.buildImplementationPrompt(feature, this.currentAnalysis, ragContext, relevantFiles);
        try {
     ...
     */
    async generateRefactoringFix(suggestion) {
        if (!this.currentAnalysis || !this.currentCodeChunks) {
            throw new Error('Codebase context not set. Run analysis first.');
        }
        ...
        const refactoringPrompt = this.buildRefactoringPrompt(suggestion, this.currentAnalysis, ragContext, targetFiles);
        try {
            const response = await this.llmProvider.generateResponse([
                {
                    role: 'system',
    ...
    buildImplementationPrompt(feature, analysis, ragContext, relevantFiles) {
...
${relevantFiles.slice(0, 5).map(file => `File: ${file.filePath}\nContent: ${file.content.substring(0, 500)}...`).join('\\n\\n')}
...
Focus on practical, implementable changes that follow the existing codebase patterns exactly.`;
    ...
    buildRefactoringPrompt(suggestion, analysis, ragContext, targetFiles) {
...
${targetFiles.map(file => `File: ${file.filePath}\nContent: ${file.content.substring(0, 800)}...`).join('\\n\\n')}
...
Return the same JSON format as feature implementation, focused on code improvements rather than new features.`;
    ...
    findRelevantFilesForFeature(feature, codeChunks) {
        ...
        const searchTerms = [
            ...
            ...feature.implementationOverview.steps.map(step => step.toLowerCase().split(' ').filter(word => word.length > 3)).flat()
        ];
        return codeChunks.filter(chunk => {
            const content = chunk.content.toLowerCase();
            const filePath = chunk.filePath.toLowerCase();
            return searchTerms.some(term => content.includes(term) || filePath.includes(term));
        }).slice(0, 10); // Limit to most relevant files
    }
    findFilesForRefactoring(suggestion, codeChunks) {
    ...
    parseImplementationResponse(response, feature) {
        try {
    ...
    generateFallbackImplementation(feature) {
        return {
            featureId: feature.id,
            title: feature.title,
    ...
    generateFallbackRefactoring(suggestion) {
    ...
    convertRefactoringToFeature(suggestion) {
        return {
...
exports.RAGImplementationService = RAGImplementationService;
//# sourceMappingURL=RAGImplementationService.js.map