...
class TutorAgent {
    ...
    async generateIntelligentTutorials(analysis, codeChunks, refactoring, architecture) {
        if (!this.llmProvider) {
        ...
        try {
            ...
            const skillContext = RAGContextBuilder_1.RAGContextBuilder.buildTutorialContext(analysis, codeChunks, refactoring, architecture);
...
Focus on advanced, practical skills that would make this developer more competitive and effective.`;
    ...
    parseIntelligentTutorials(llmResponse) {
        try {
            ...
            return tutorials.filter(t => t.id &&
                t.title &&
                t.description &&
                t.difficulty &&
                t.relevanceScore &&
    ...
    analyzeLearningNeeds(codeChunks, analysis) {
        ...
        analysis.key_technologies.forEach(tech => {
            const techUsage = codeChunks.filter(chunk => chunk.content.toLowerCase().includes(tech.toLowerCase())).length;
            if (techUsage > 5) {
        ...
        const hasModernJS = codeChunks.some(chunk => chunk.content.includes('async') || chunk.content.includes('await'));
        if (!hasModernJS) {
    ...
    generateCuratedTutorials(query) {
        const tutorials = [];
