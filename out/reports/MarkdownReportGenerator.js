...
exports.MarkdownReportGenerator = void 0;
...
${results.analysis.potential_areas_for_refactoring.map((area, index) => `${index + 1}. **${area}**\n   - Impact: ${this.assessRefactoringImpact(area)}\n   - Effort: ${this.assessRefactoringEffort(area)}`).join('\n\n')}`;
    }
    generateRefactoringSection(results) {
        ...
        const suggestions = results.refactoring.suggestions.map((suggestion, index) => {
...
${suggestions}`;
    ...
    generateArchitectureSection(results) {
        ...
        const features = results.architecture.features.map((feature, index) => {
...
${feature.implementationOverview.technologies.map((tech) => `- ${tech}`).join('\n')}
...
**🎯 Primary Use Cases:**
${lib.useCases.slice(0, 3).map((useCase) => `- ${useCase}`).join('\n')}

**💡 Key Benefits:**
${lib.benefits.slice(0, 3).map((benefit) => `- ${benefit}`).join('\n')}
...
${tutorials}`;
    ...
    generateStrategicInsights(results) {
        ...
        if (results.enhanced.uniqueRecommendations && results.enhanced.uniqueRecommendations.length > 0) {
            const recommendations = results.enhanced.uniqueRecommendations.map((rec) => {
                return `### ${rec.title}
...
${recommendations}`);
        ...
        if (results.enhanced.competitiveAnalysis) {
            const opportunities = results.enhanced.competitiveAnalysis.innovationOpportunities.map((opp) => {
...
**Key Differentiators:**
${strategy.marketPositioning.differentiators.map((diff) => `- ${diff}`).join('\n')}
...
*🤖 Powered by AI-driven code analysis*`;
    ...
    generateQualityTrendAnalysis(results) {
        const metrics = results.analysis.code_quality_metrics;
    ...
    getComplexityFactors(score) {
        const factors = [];
        if (score >= 7)
            factors.push('High cyclomatic complexity');
        if (score >= 6)
            factors.push('Deep nesting levels');
        if (score >= 5)
            factors.push('Large function/class sizes');
        if (score >= 4)
            factors.push('Multiple responsibilities');
        return factors.length > 0 ? factors.join(', ') : 'Well-structured code';
    }
    assessRefactoringImpact(area) {
        // Simple heuristic based on area type
        if (area.toLowerCase().includes('performance'))
            return 'High';
        if (area.toLowerCase().includes('security'))
            return 'Critical';
        if (area.toLowerCase().includes('maintainability'))
            return 'Medium';
        return 'Medium';
    }
    assessRefactoringEffort(area) {
        if (area.toLowerCase().includes('architecture'))
            return 'High';
        if (area.toLowerCase().includes('refactor'))
            return 'Medium';
        return 'Low';
    }
    ...
    generateBusinessReport(results) {
        return this.generate(results, {
...
exports.MarkdownReportGenerator = MarkdownReportGenerator;
