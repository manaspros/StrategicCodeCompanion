...
class DependencyGraphProvider {
    ...
    async showDependencyGraph(workspacePath) {
        // Create webview panel
        this.panel = vscode.window.createWebviewPanel('dependencyGraph', 'Code Dependencies - Strategic Code Companion', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        try {
            ...
            this.panel.webview.onDidReceiveMessage(message => this.handleWebviewMessage(message, graph), undefined, this.context.subscriptions);
        }
        catch (error) {
    ...
    async analyzeDependencies(workspacePath) {
        const ingestion = new ingestion_1.CodeIngestion(workspacePath);
    ...
    findDependencyNodeId(dependency, nodes) {
    ...
    determineEdgeType(chunk, dependency) {
        const content = chunk.content.toLowerCase();
    ...
    handleWebviewMessage(message, graph) {
        switch (message.type) {
            case 'nodeClicked':
    ...
    async handleNodeClick(nodeId, graph) {
    ...
    async exportGraph(graph, format) {
    ...
    graphToCsv(graph) {
        ...
        const rows = graph.edges.map(edge => `${edge.from},${edge.to},${edge.type},${edge.weight}`).join('\n');
        return header + rows;
    }
    ...
    async showCircularDependencies(cycles) {
        if (cycles.length === 0) {
        ...
        if (selected) {
            ...
            vscode.window.showWarningMessage(`Circular dependency: ${selected.cycle.length} files involved`, 'View Details', 'Fix Suggestions').then(choice => {
    ...
    showCircularDependencyFixes() {
        ...
        vscode.window.showInformationMessage(`Suggestions to fix circular dependencies:\n\n${suggestions.join('\n')}`, { modal: true });
    }
    getLoadingHtml() {
        return `<!DOCTYPE html>
        <html>
    ...
    getDependencyGraphHtml(graph, filters) {
